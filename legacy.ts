...
var progressChart2 = document.getElementById('progressChart2'),
    progressctx = progressChart2.getContext('2d');
...
function fn_BuildDataWeighted(para_GuildName, para_InputJSON) {
    ...
    for (var i = para_InputJSON.length - 1; i >= 0; i--) {
        ...
        //for each row item
        for (var x = HighScoreSet.result.extractorData.data["0"].group.length - 1; x >= 0; x--) {
            if(! HighScoreSet.result.extractorData.data["0"].group[x].Guild) { //if undefined guild
                guild = "(Guildless)";
            } else {
                guild = HighScoreSet.result.extractorData.data["0"].group[x].Guild["0"].text;
            }
...
function fn_MakeRGB(para_Input,para_Guild = "alf") {
    //"rgba(3,3,3,1)"
